...
def layers(inputs, params=None):
    ...
    pipe = DepthwiseConv3D(kernel_size=(1,4,4), strides=(1,1,1), depth_multiplier=32, padding='valid', groups=params['n_channels'])(inputs)
    pipe = BatchNormalization()(pipe)
    pipe = LeakyReLU(alpha=0.05)(pipe)
    pipe = DepthwiseConv3D(kernel_size=(1,2,3), strides=(1,1,1), depth_multiplier=32, padding='valid', groups=params['n_channels'])(pipe)
    pipe = LeakyReLU(alpha=0.05)(pipe)
