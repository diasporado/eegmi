...
def layers(inputs):
    pipe = Reshape((inputs.shape[1].value, 1, 1, inputs.shape[2].value))(inputs)
    pipe = DepthwiseConv3D(kernel_size=(25,1,1), strides=(2,1,1), depth_multiplier=40, groups=22)(pipe)
    pipe = Reshape((pipe.shape[1].value, pipe.shape[4].value, 1))(pipe)
    pipe = Convolution2D(40, (1,22*40), strides=(1,1))(pipe)
